app.controller('MakeAndModelManagementCtrl',
    function ($modal, $scope, $rootScope, $http, SweetAlert, $localStorage, ngDialog, Idle, Keepalive, $filter, toaster, $interval) {
                ...
                if (!isGuid($scope.Variant.Id)) {
                    angular.forEach($scope.Variants, function (variant) {
                        if (variant.VariantName.trim() === $scope.Variant.VariantName.trim()) {
                            $scope.validate_VariantName = "has-error";
                            isValid = false;
                            customErrorMessage("The variant name already exist.")
                            return false;
                        }
                    });
                }
        ...
        $scope.VariantSubmit = function () {
            ...
            if ($scope.validateVariant()) {
                ...
                if ($scope.Variant.ToModelYear == undefined || $scope.Variant.ToModelYear == null || $scope.Variant.ToModelYear == "") {
                    ...
                    customErrorMessage("Enter From Year As current year or beyond");
                    return false;
                }
                if ($scope.Variant.FromModelYear == undefined || $scope.Variant.FromModelYear == null || $scope.Variant.FromModelYear == "" ||
                    ...
                    customErrorMessage("To year should be larger than from year");
                    return false;
                }
        ...
        $scope.SetManufacturerWarrantyValues = function () {
            ...
            if ($scope.ManufacturerWarranty.Id != null) {
                $http({
                    ...
                    // $scope.ManufacturerWarranty.ModelId = data.ModelId;
                    $scope.ManufacturerWarranty.WarrantyKm = data.WarrantyKm;
                    $scope.Expired = data.Expired;

                    //angular.forEach(data.Models, function (value) {
                    //    var selectedModels = {
                    //        id: value
                    //    };
                    //    $scope.SelectedModelList.push(selectedModels)
                    //});

                    //$scope.selectedModelChanged();

                    //angular.forEach(data.Countrys, function (value) {
                    //    var selectedCountrys = {
                    //        id: value
                    //    };
                    //    $scope.SelectedCountryListM.push(selectedCountrys)
                    //});
            ...
            if ($scope.WarantyKm) {
                ...
                angular.forEach($scope.ManufacturerWarranties, function (valmw) {
                    if ($scope.ManufacturerWarranty.WarrantyName == valmw.WarrantyName) {
                        ...
                        isValid = false;
                        customErrorMessage("This Manufacturer Warranty Name Alrady Enterd.")
                    }
        ...
        $scope.ManufacturerWarrantySubmit = function () {
            if ($scope.validateManufacturerWarranty()) {
                ...
                if ($scope.ManufacturerWarranty.Id == null || $scope.ManufacturerWarranty.Id == "00000000-0000-0000-0000-000000000000") {
                    swal({ title: 'Reading...', text: 'Manufacturer Warranty Information', showConfirmButton: false });
                    angular.forEach($scope.ManufacturerWarranties, function (valmw) {
                        if ($scope.ManufacturerWarranty.WarrantyName == valmw.WarrantyName) {
                            isValid = false;
                            customErrorMessage("This Manufacturer Warranty Name Alrady Enterd.")
        ...
        $scope.WarantyDuration = "Warranty Months";
            ...
            angular.forEach($scope.CommodityTypes, function (valc) {
                if ($scope.ManufacturerWarranty.CommodityTypeId == valc.CommodityTypeId) {
                    if (valc.CommodityTypeDescription == "Automobile") {
                        ...
                        $scope.WarantyDuration = "Warranty Hours";
                    }
            ...
            if ($scope.ManufacturerWarranty.ModelId != null) {
                $http({
                    ...
                    if ($scope.MWCountries.length == 0) {
                        customErrorMessage("Please add the Make and Model selected to the relavant country in order to load the country list.")
        ...
        $scope.gridManufacturerWarranty = {
            ...
            columnDefs: [
                ...
                { name: 'Make', field: 'Makes', enableSorting: false, cellClass: 'columCss',width:100 },
                { name: 'Warranty Name', field: 'WarrantyName', enableSorting: false, cellClass: 'columCss', width: 115 },
                { name: 'Warranty Months', field: 'WarrantyMonths', enableSorting: false, cellClass: 'columCss', width: 125 },
                { name: 'WarrantyKm', field: 'WarrantyKm', enableSorting: false, cellClass: 'columCss', width: 100 },
                { name: 'ApplicableFrom', field: 'ApplicableFrom', enableSorting: false, cellClass: 'columCss', width: 120 },
                ...
                {
                    name: ' ',
                    cellTemplate: '<div class="center"><button ng-click="grid.appScope.loadManufacturerWarrantyForUpdate(row.entity.Id)" class="btn btn-xs btn-warning">Update</button></div>',
                    width: 60,
                    enableSorting: false
                }
            ],
