    ...
    function ($scope, $rootScope, $http, SweetAlert, $localStorage, toaster) {
            ...
            if ($scope.DealerLocation.DealerId != null) {
                ...
                angular.forEach($scope.Dealers, function (value, key) {
                    if (value.Id == $scope.DealerLocation.DealerId) {
                        ...
                        swal({ title: 'Loading...', text: 'Dealer  Loading', showConfirmButton: false });
                        $http({
                            method: 'POST',
        ...
        $scope.SetDealerLocationValues = function () {
            ...
            if ($scope.DealerLocation.Id != null) {
                 swal({ title: 'Loading...', text: 'Dealer Branch Loading', showConfirmButton: false });
                $http({
                    method: 'POST',
        ...
        function CheckHeadOfficeBranch() {
            angular.forEach($scope.DealerLocations, function (value) {
                if ($scope.DealerLocation.HeadOfficeBranch && value.Id != $scope.DealerLocation.Id &&
                    ...
                    SweetAlert.swal({
                        title: "Dealer Branch Information",
                        text: "Head Office Branch Already Created",
                        type: "warning",
                        confirmButtonColor: "rgb(221, 107, 85)"
                    });
                    $scope.DealerLocation.HeadOfficeBranch = false;
        ...
        $scope.DealerLocationSubmit = function () {
            ...
            if ($scope.validateDealerBranch()) {
                ...
                //if (!$scope.DealerLocation.SalesTelephone.match(phoneno) ||
                //    !$scope.DealerLocation.SalesFax.match(phoneno) ||
                //    !$scope.DealerLocation.ServiceTelephone.match(phoneno) ||
                //   !$scope.DealerLocation.ServiceFax.match(phoneno)
                //    ) {
                //    $scope.errorTab2 = "Please enter valid Telephone / Fax No: Length should be greater than or equal to 10 and less than 16";
                //    return false;
                //}
                //CheckHeadOfficeBranch();
                $scope.errorTab2 = "";
                //if (!CheckHeadOfficeBranch()) {
                //    //!IsHeadOfficeBranch
                //    return;
                //}
        ...
        $scope.dealerLabourChargeSubmit = function () {
            if ($scope.validateDealerLabourCharge()) {
                $http({
                    ...
                    if (data == "ok") {
                        SweetAlert.swal({
                            title: "Dealer Labour Charge Information",
                            text: "Successfully Saved!",
                            confirmButtonColor: "#007AFF"
                        });
                        clearDealerLabourChargeControls();
        ...
        $scope.gridDealerLabourCharge = {
            ...
            columnDefs: [
              { name: 'Id', field: 'Id', enableSorting: false, visible: false, cellClass: 'columCss' },
                { name: 'Dealer', field: 'Dealers', enableSorting: false, cellClass: 'columCss'},
              { name: 'Country', field: 'Country', enableSorting: false, cellClass: 'columCss', },
              { name: 'Make', field: 'Makes', enableSorting: false, cellClass: 'columCss' },
              { name: 'Model', field: 'Models', enableSorting: false, cellClass: 'columCss' },
              { name: 'Start Date', field: 'StartDate', enableSorting: false, cellClass: 'columCss', minWidth: 90 },
              { name: 'End Date', field: 'EndDate', enableSorting: false, cellClass: 'columCss', minWidth: 90 },
              { name: 'Currency', field: 'Currencys', enableSorting: false, cellClass: 'columCss' },
              { name: 'LabourChargeValue', field: 'LabourChargeValue', enableSorting: false, cellClass: 'columCss' },

              {
                  name: ' ',
                  cellTemplate: '<div class="center"><button ng-click="grid.appScope.loadDealerLabourChargeForUpdate(row.entity.Id)" class="btn btn-xs btn-warning">Update</button></div>',
                  width: 60,
                  enableSorting: false
              }
            ],
        ...
        $scope.loadDealerLabourChargeForUpdate = function (dealerLabourChargeId) {
            if (isGuid(dealerLabourChargeId)) {
                swal({ title: 'Reading...', text: 'Dealer Labour Charge information', showConfirmButton: false });
                $http({
                    method: 'POST',
