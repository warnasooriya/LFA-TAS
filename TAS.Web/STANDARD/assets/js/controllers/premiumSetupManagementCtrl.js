app.controller('PremiumSetupManagementCtrl',
    function ($scope, $rootScope, $http, SweetAlert, ngDialog, $localStorage, $timeout, $interval, $state, toaster, $filter, $translate) {
        ...
        $scope.settings = { scrollable: true, scrollableHeight: 400, enableSearch: true  };
        ...
        $scope.IsAEWTyreProduct = false;


        $scope.inizializedDataForReuse = {
            countries:[],
            Allcountries:[],
            dealers: [],
            products: [],
        ...
        $scope.claimLimitationCriteria = {
            enabled : false
        }
        $scope.claimLimitationListOriginal = [];
        $scope.claimLimitationList=[];
        $scope.claimLimitationErrors = "";
        $scope.generateClaimCriteria = function () {
            $scope.claimLimitationList = [];
            if ($scope.claimLimitationCriteria.enabled) {

                $scope.claimLimitationErrors = "";
                let startDate = $scope.currentContract.contractStartDate;
                let endDate = $scope.currentContract.contractEndDate;
                if (startDate == undefined || startDate == null || startDate.length == 0 || endDate.length == 0 || endDate == undefined || endDate==null) {
                    $scope.claimLimitationErrors = $filter('translate')('pages.premiumSetupManagement.errorMessages.selectContract');
                    $scope.claimLimitationCriteria.enabled = false;
                    return false;
                }
                if ($scope.claimLimitationListOriginal.length > 0) {
                    $scope.claimLimitationList = $scope.claimLimitationListOriginal;
                }
                let startD = new Date(startDate);
                let EndD = new Date(endDate);
                let total_months = (EndD.getFullYear() - startD.getFullYear()) * 12 + (EndD.getMonth() - startD.getMonth())
                var years = Math.ceil(total_months / 12);
                if ($scope.claimLimitationList.length == 0) {
                    for (x = 1; x <= years; x++) {
                        let lbl = "Year " + x;
                        let obj = { id: '00000000-0000-0000-0000-000000000000', year: x, label: lbl, presentage: 0 }
                        $scope.claimLimitationList.push(obj)
                    }
                }
            }
        ...
        $scope.currencyPeriodCheck = function () {
            $http({
                ...
                if (data == true) {
                    ...
                    swal({ title: $filter('translate')('pages.premiumSetupManagement.errorMessages.tASSecurity'), text: $filter('translate')('pages.premiumSetupManagement.errorMessages.noCurencyPeriod'), showConfirmButton: false });
                    setTimeout(function () { swal.close(); }, 5000);
                    $state.go('app.dashboard');
                } else {
                    swal({ title: $filter('translate')('pages.premiumSetupManagement.errorMessages.tASSecurity'), text: $filter('translate')('pages.premiumSetupManagement.errorMessages.errorCurency'), showConfirmButton: false });
                    setTimeout(function () { swal.close(); }, 5000);
                    $state.go('app.dashboard');
                }
        ...
        $scope.loadInitailData = function () {
            ...
            $http({
                ...
                $scope.inizializedDataForReuse.countries = data;
                $scope.inizializedDataForReuse.Allcountries = data;
            ...
            $http({
                method: 'POST',
                url: '/TAS.Web/api/ApiManagerForPolicyApproval/GetCountryDealerProductCommoditiesInsures',
                headers: { 'Authorization': $localStorage.jwt == undefined ? jwt : $localStorage.jwt }
            }).success(function (data, status, headers, config) {

                $scope.dealers = data.dealers;
                $scope.inizializedDataForReuse.dealers = data.dealers;

                $scope.products = data.products;
                $scope.inizializedDataForReuse.products = data.products;

                $scope.commodityTypes = data.commodityTypes;
                $scope.inizializedDataForReuse.commodityTypes = data.commodityTypes;

                $scope.insurers = data.insurers;
                $scope.inizializedDataForReuse.insurers = data.insurers;
            ...
            $http({
                method: 'POST',
                url: '/TAS.Web/api/ApiManagerForPolicyApproval/GetAllItemStatussCylinderCountEngineCapacityVehicleWeightBasedOns',
                headers: { 'Authorization': $localStorage.jwt == undefined ? jwt : $localStorage.jwt }
            }).success(function (data, status, headers, config) {
                $scope.itemStatuses = data.itemStatuses;
                $scope.inizializedDataForReuse.itemStatuses = data.itemStatuses;

                $scope.clinderCounts = data.clinderCounts;
                $scope.inizializedDataForReuse.clinderCounts = data.clinderCounts;
                $scope.refreshCyllinderCountwithUI();

                $scope.engineCapacities = data.engineCapacities;
                $scope.inizializedDataForReuse.engineCapacities = data.engineCapacities;
                $scope.refreshCCwithUI();

                $scope.grossVehicleWeights = data.grossVehicleWeights;
                $scope.inizializedDataForReuse.grossVehicleWeights = data.grossVehicleWeights;
                $scope.refreshWWwithUI();

                $scope.premiumBasedOnValues = data.premiumBasedOnValues;
                $scope.inizializedDataForReuse.premiumBasedOnValues = data.premiumBasedOnValues;
            ...
            $http({
                method: 'POST',
                url: '/TAS.Web/api/ApiManagerForPolicyApproval/GetWarrantyTypesRSAProviderRegionsCommissionTypesCommodityUsageTypesDealTypes',
                headers: { 'Authorization': $localStorage.jwt == undefined ? jwt : $localStorage.jwt }
            }).success(function (data, status, headers, config) {
                $scope.warrantyTypes = data.warrantyTypes;
                $scope.inizializedDataForReuse.warrantyTypes = data.warrantyTypes;

                $scope.rsaProviders = data.rsaProviders;
                $scope.inizializedDataForReuse.rsaProviders = data.rsaProviders;

                $scope.regions = data.regions;
                $scope.inizializedDataForReuse.regions = data.regions;


                $scope.commissionTypes = data.commissionTypes;
                $scope.inizializedDataForReuse.commissionTypes = data.commissionTypes;
                angular.forEach($scope.commissionTypes, function (value) {
                    value.IsOnNRP = false;
                    value.IsOnGROSS = false;
                });

                $scope.commodityUsageTypes = data.commodityUsageTypes;
                $scope.inizializedDataForReuse.commodityUsageTypes = data.commodityUsageTypes;

                $scope.dealTypes = data.dealTypes;
                $scope.inizializedDataForReuse.dealTypes = data.dealTypes;
        ...
        $scope.loadInitailDataOld = function () {
            $http({
                method: 'POST',
                url: '/TAS.Web/api/ContractManagement/GetAllCountries',
                headers: { 'Authorization': $localStorage.jwt == undefined ? jwt : $localStorage.jwt }
            ...
            $http({
                ...
                $scope.dealers = data;
                $scope.inizializedDataForReuse.dealers = data;
            }).error(function (data, status, headers, config) {
            });
            $http({
                method: 'POST',
                ...
                $scope.products = data;
                $scope.inizializedDataForReuse.products = data;
            }).error(function (data, status, headers, config) {
            });
            ...
            $http({
                ...
                $scope.inizializedDataForReuse.commodityTypes = data;
            }).error(function (data, status, headers, config) {
            });
            ...
            $http({
                ...
                $scope.insurers = data;
                $scope.inizializedDataForReuse.insurers = data;
            }).error(function (data, status, headers, config) {
            });
            ...
            $http({
                ...
                $scope.inizializedDataForReuse.itemStatuses = data;
            }).error(function (data, status, headers, config) {
            });
            ...
            $http({
                ...
                $scope.inizializedDataForReuse.clinderCounts = data;
                // console.log(data);
                $scope.refreshCyllinderCountwithUI();
            }).error(function (data, status, headers, config) {
            });
            $http({
                ...
                $scope.inizializedDataForReuse.engineCapacities = data;
                //  console.log(data);
                $scope.refreshCCwithUI();
            }).error(function (data, status, headers, config) {
            });
            ...
            $http({
                ...
                $scope.inizializedDataForReuse.grossVehicleWeights = data;
                // console.log(data);
                $scope.refreshWWwithUI();
            }).error(function (data, status, headers, config) {
            });
            ...
            $http({
                ...
                $scope.inizializedDataForReuse.premiumBasedOnValues = data;
            }).error(function (data, status, headers, config) {
            });
            $http({
                method: 'POST',
                ...
                $scope.inizializedDataForReuse.warrantyTypes = data;
            }).error(function (data, status, headers, config) {
            });
            ...
            $http({
                ...
                $scope.inizializedDataForReuse.rsaProviders = data;
            ...
            $http({
                ...
                $scope.regions = data;
                $scope.inizializedDataForReuse.regions = data;
            }).error(function (data, status, headers, config) {
            });
            ...
            $http({
                ...
                $scope.inizializedDataForReuse.commissionTypes = data;
                angular.forEach($scope.commissionTypes, function (value) {
                    value.IsOnNRP = false;
            ...
            $http({
                ...
                $scope.inizializedDataForReuse.commodityUsageTypes = data;
            }).error(function (data, status, headers, config) {
            });
            ...
            $http({
                ...
                $scope.inizializedDataForReuse.dealTypes = data;
            }).error(function (data, status, headers, config) {
            });
        ...
        $scope.loadInitailDataWithLoadedData = function () {
            $scope.countries = $scope.inizializedDataForReuse.countries;
            $scope.Allcountries = $scope.inizializedDataForReuse.Allcountries;
            $scope.dealers = $scope.inizializedDataForReuse.dealers;
            $scope.products = $scope.inizializedDataForReuse.products;
            $scope.commodityTypes = $scope.inizializedDataForReuse.commodityTypes;
            $scope.insurers = $scope.inizializedDataForReuse.insurers;
            $scope.itemStatuses = $scope.inizializedDataForReuse.itemStatuses;
            $scope.clinderCounts = $scope.inizializedDataForReuse.clinderCounts;
            $scope.commissionTypes = $scope.inizializedDataForReuse.commissionTypes;
            $scope.engineCapacities = $scope.inizializedDataForReuse.engineCapacities;
            $scope.grossVehicleWeights = $scope.inizializedDataForReuse.grossVehicleWeights;
            $scope.premiumBasedOnValues = $scope.inizializedDataForReuse.premiumBasedOnValues;
            $scope.warrantyTypes = $scope.inizializedDataForReuse.warrantyTypes;
            $scope.rsaProviders = $scope.inizializedDataForReuse.rsaProviders;
            $scope.regions = $scope.inizializedDataForReuse.regions;
            $scope.commodityUsageTypes = $scope.inizializedDataForReuse.commodityUsageTypes;
            $scope.dealTypes = $scope.inizializedDataForReuse.dealTypes;
            $scope.refreshCyllinderCountwithUI();
            $scope.refreshCCwithUI();
            $scope.refreshWWwithUI();
        ...
        $scope.updateContractStatus = function (id, isActive) {
            ...
            if (!isActive)
                status = "active";
            swal({
                title: $filter('translate')('pages.premiumSetupManagement.errorMessages.areYouSure'),
                text: $filter('translate')('pages.premiumSetupManagement.errorMessages.selectedContract') + status,
                type: "warning",
                ...
                if (isGuid(id)) {
                    $http({
                        ...
                        if (data) {
                            ...
                            SweetAlert.swal({
                                title: $filter('translate')('common.alertTitle'),
                                text: $filter('translate')('pages.premiumSetupManagement.errorMessages.errorOccured'),
                                type: "warning",
                                confirmButtonText: $filter('translate')('common.button.ok'),
        ...
        $scope.contractSearchGrid = {
            ...
            columnDefs: [
                ...
                { name: $filter('translate')('pages.premiumSetupManagement.commodityType'), width: '18%', field: 'CommodityType', enableSorting: false, cellClass: 'columCss' },
                { name: $filter('translate')('pages.premiumSetupManagement.country'), field: 'CountryName', width: '16%', enableSorting: false, cellClass: 'columCss' },
                { name: $filter('translate')('pages.premiumSetupManagement.dealer'), field: 'DealerName', width: '16%', enableSorting: false, cellClass: 'columCss' },
                { name: $filter('translate')('pages.premiumSetupManagement.product'), field: 'Productname', width: '16%',enableSorting: false, cellClass: 'columCss' },
                { name: $filter('translate')('pages.premiumSetupManagement.dealName'), field: 'DealName', width: '16%', enableSorting: false, cellClass: 'columCss' },
                {
                    name: '   ',
                    cellTemplate: '<div class="center"><button ng-click="grid.appScope.updateValidityCheck(row.entity.Id)" class="btn btn-xs btn-info">' + $filter('translate')('common.button.update') +'</button></div>',
                    width: 60,
                    enableSorting: false
                }
                ,
        ...
        $scope.updateValidityCheck = function (contractIdToUpdate) {
            if (isGuid(contractIdToUpdate)) {
                $scope.saveBtnDisabled = false;
                $scope.createNewDeal = false;
                swal({ title: $filter('translate')('common.alertTitle'), text: $filter('translate')('pages.premiumSetupManagement.errorMessages.loadingDeal'), showConfirmButton: false });
                $http({
                    method: 'POST',
                    ...
                    if (!data) {
                        customWarningMessage($filter('translate')('pages.premiumSetupManagement.errorMessages.selectedContractLinked'));
                ...
                SweetAlert.swal({
                    title: $filter('translate')('common.alertTitle'),
                    text: $filter('translate')('pages.premiumSetupManagement.errorMessages.invalidContractSelection'),
                    type: "warning",
                    confirmButtonText: $filter('translate')('common.button.ok'),
                    confirmButtonColor: "rgb(221, 107, 85)"
                });
        ...
        $scope.loadContract = function (contractId) {
            if (isGuid(contractId)) {
                ...
                $http({
                    ...
                    // claim criteria loads
                    if (data.data.contract.claimCriteria != undefined && data.data.contract.claimCriteria.length > 0) {
                        $scope.claimLimitationList = data.data.contract.claimCriteria;
                        $scope.claimLimitationListOriginal = data.data.contract.claimCriteria;
                        $scope.claimLimitationCriteria.enabled = true;
                    } else {
                        $scope.claimLimitationListOriginal = [];
                        $scope.claimLimitationList = [];
                        $scope.claimLimitationCriteria.enabled = false;
                            ...
                            $http({
                                ...
                                $http({
                                    ...
                                    if (data == false) {
                                        SweetAlert.swal({
                                            title: $filter('translate')('common.alertTitle'),
                                            text: $filter('translate')('pages.premiumSetupManagement.errorMessages.selectedDealer') + $scope.premiumCurrency + $filter('translate')('pages.premiumSetupManagement.errorMessages.notDefinedCurrency'),
                                            type: "warning",
                                            confirmButtonText: $filter('translate')('common.button.ok'),
                                            confirmButtonColor: "rgb(221, 107, 85)"
                                        });
        ...
        $scope.selectedCommodityChanged = function () {
            console.log($scope.currentContract.commodityTypeId);
            $scope.IsTyreProduct = false;
            angular.forEach($scope.commodityTypes, function (commodityType) {
                if (commodityType.CommodityTypeId === $scope.currentContract.commodityTypeId) {
                    if (commodityType.CommodityTypeDescription == "Tyre") {
                        $scope.IsTyreProduct = true;
                    }
                }
            });
        ...
        $scope.selectedProductChanged = function () {
            ...
            for (var i = 0; i < $scope.commodityProducts.length; i++) {
                if ($scope.commodityProducts[i].Id === $scope.currentContract.productId) {
                    ...
                    if ($scope.currentProductType == "ADT") {
                        $scope.IsAEWTyreProduct = true;
                    }
            ...
            if (isGuid($scope.currentContract.commodityTypeId)) {
                if ($scope.validateNewInsuaranceLimitation()) {
                    ...
                    $http({
                        ...
                        if (data.code === "ok") {
                            $scope.setNewInsuaranceLimitationMsg = "";
                            customInfoMessage($filter('translate')('pages.premiumSetupManagement.errorMessages.limitationSaved'));
                ...
                customErrorMessage($filter('translate')('pages.premiumSetupManagement.errorMessages.selectedCommodityType'));
            }
            ...
            if ($scope.IsAutomobile) {
                ...
                if (
                    ...
                    if ((parseInt($scope.eligibility.ageMin) > parseInt($scope.eligibility.ageMax))
                        ...
                        $scope.eligibilityError = $filter('translate')('pages.premiumSetupManagement.errorMessages.minimumValues');
                    } else {
                        if ($scope.eligibility.isPresentage) {
                            if ($scope.eligibility.premium > 100) {
                                $scope.eligibilityError = $filter('translate')('pages.premiumSetupManagement.errorMessages.percentageValue');
                            } else {
                                $scope.eligibilityError = "";
                    ...
                    $scope.eligibilityError = $filter('translate')('pages.premiumSetupManagement.errorMessages.validValues');
        ...
        $scope.savePremiumSetup = function () {
            if ($scope.createNewDeal) {
                //add new premium
                $scope.contractIdToUpdate = emptyGuid();
                console.log($scope.contractIdToUpdate);
                if (!$scope.validateGeneralDetails(false) &&
                    ...
                    $scope.currentContract.claimCriteria = $scope.claimLimitationList;
                    var data = {
                    ...
                    //customErrorMessage($filter('translate')('pages.premiumSetupManagement.errorMessages.fillAllHighlightedFields'));
                }
                //return false;
            } else {
                //update premium
        ...
        $scope.validateGeneralDetails = function (isUpdate) {
            ...
            if (IsDate($scope.currentContract.contractEndDate) && IsDate($scope.currentContract.contractStartDate)) {
                if ((new Date($scope.currentContract.contractStartDate).getTime() > new Date($scope.currentContract.contractEndDate).getTime())) {
                    $scope.validate_endDate = "has-error";
                    $scope.validate_startDate = "has-error";
                    $scope.dateValidationError = $filter('translate')('pages.premiumSetupManagement.errorMessages.startDateLess');
                    haveErrorInputs = true;
                    customErrorMessage($filter('translate')('pages.premiumSetupManagement.errorMessages.fillAllHighlightedFields'));
                } else {
                ...
                angular.forEach($scope.allChildProducts, function (child) {
                    ...
                    if (child.IsMinMaxVisibleNett) {
                        ...
                        if (parseFloat(child.maxValueNett) < parseFloat(child.minValueNett)) {
                            ...
                            child.validate_maxValueNett = "has-error";
                            child.validate_minValueNett = "has-error";
                            child.minMaxNettError = $filter('translate')('pages.premiumSetupManagement.errorMessages.exceedMaxValue');
                            customErrorMessage($filter('translate')('pages.premiumSetupManagement.errorMessages.fillAllHighlightedFields'));
                        } else {
                            child.validate_maxValueNett = "";
                            child.validate_minValueNett = "";
                        ...
                        if (!parseFloat(child.totalNRP)) {
                            customErrorMessage($filter('translate')('pages.premiumSetupManagement.errorMessages.nrpCannotZero'));
                            haveErrorInputs = true;
                        }
                    }
                    if ($scope.ValidategrossandnrpValues()) {
                        ...
                        haveErrorInputs = true;
                        customErrorMessage($filter('translate')('pages.premiumSetupManagement.errorMessages.markCommission'));
                    ...
                    if (child.IsMinMaxVisibleGross) {
                        ...
                        if (parseFloat(child.maxValueGross) < parseFloat(child.minValueGross)) {
                            ...
                            child.validate_maxValueGross = "has-error";
                            child.validate_minValueGross = "has-error";
                            child.minMaxGrossError = $filter('translate')('pages.premiumSetupManagement.errorMessages.exceedMaxValue');
                            customErrorMessage($filter('translate')('pages.premiumSetupManagement.errorMessages.fillAllHighlightedFields'));
                        } else {
                            child.validate_maxValueGross = "";
                            child.validate_minValueGross = "";
                        ...
                        if (!parseFloat(child.grossPremium)) {
                            customErrorMessage($filter('translate')('pages.premiumSetupManagement.errorMessages.grossPremiumZero'));
                            haveErrorInputs = true;
                        }
                    }
        ...
        $scope.validatePremiumDetails = function () {
            ...
            if ($scope.IsMinMaxVisibleNett) {
                ...
                if (!haveErrorInputs) {
                    if (parseFloat($scope.currentContract.minValueNett) > parseFloat($scope.currentContract.maxValueNett)) {
                        $scope.validate_minValueNett = "has-error";
                        $scope.validate_maxValueNett = "has-error";
                        $scope.minMaxNettError = $filter('translate')('pages.premiumSetupManagement.errorMessages.exceedMaxValue')
                        haveErrorInputs = true;
                    } else {
                        $scope.validate_minValueNett = "";
                        $scope.validate_maxValueNett = "";
            ...
            if ($scope.IsMinMaxVisibleGross) {
                ...
                if (!haveErrorInputs) {
                    if (parseFloat($scope.currentContract.minValueGross) > parseFloat($scope.currentContract.maxValueGross)) {
                        $scope.validate_minValueGross = "has-error";
                        $scope.validate_maxValueGross = "has-error";
                        $scope.minMaxGrossError = $filter('translate')('pages.premiumSetupManagement.errorMessages.exceedMaxValue')
                        haveErrorInputs = true;
                    } else {
                        $scope.validate_minValueGross = "";
                        $scope.validate_maxValueGross = "";
            ...
            if (parseFloat($scope.totalNRP) > 0 && parseFloat($scope.totalNRP) > 0) {
                ...
                $scope.GeneralError = $filter('translate')('pages.premiumSetupManagement.errorMessages.premiumCannotZero');
                haveErrorInputs = true;
            }
        ...
        $scope.setupNewContract = function () {
            ...
            $scope.isAllCyllinderCountSelected = false;
            $scope.isAllEngineCapacitySelected = false;
            $scope.loadInitailDataWithLoadedData();
