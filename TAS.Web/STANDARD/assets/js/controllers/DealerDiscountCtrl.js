    ...
    function ($scope, $rootScope, $http, SweetAlert, $localStorage, toaster, $filter, $translate) {
        ...
        $scope.gridDealerDiscounts = {
            ...
            columnDefs: [
              { name: 'Id', field: 'Id', enableSorting: false, visible: false, cellClass: 'columCss' },
                { name: $filter('translate')('pages.dealerDiscountManagement.type'), field: 'Type', enableSorting: false, cellClass: 'columCss' },
                { name: $filter('translate')('pages.dealerDiscountManagement.country'), field: 'Country', enableSorting: false, cellClass: 'columCss', },
                { name: $filter('translate')('pages.dealerDiscountManagement.dealer'), field: 'Dealer', enableSorting: false, cellClass: 'columCss' },
                { name: $filter('translate')('pages.dealerDiscountManagement.make'), field: 'Make', enableSorting: false, cellClass: 'columCss' },
                { name: $filter('translate')('pages.dealerDiscountManagement.from'), field: 'From', enableSorting: false, cellClass: 'columCss', minWidth: 90 },
                { name: $filter('translate')('pages.dealerDiscountManagement.to'), field: 'To', enableSorting: false, cellClass: 'columCss', minWidth: 90 },
                { name: $filter('translate')('pages.dealerDiscountManagement.discount'), field: 'Discount', enableSorting: false, cellClass: 'columCss' },
                { name: $filter('translate')('pages.dealerDiscountManagement.goodwill'), field: 'Goodwill', enableSorting: false, cellClass: 'columCss' },
                { name: $filter('translate')('pages.dealerDiscountManagement.scheme'), field: 'Scheme', enableSorting: false, cellClass: 'columCss' },
                { name: $filter('translate')('pages.dealerDiscountManagement.active'), field: 'Active', enableSorting: false, cellClass: 'columCss', width: 60 },

              {
                  name: ' ',
                  cellTemplate: '<div class="center"><button ng-click="grid.appScope.loadDealerDiscountForUpdate(row.entity.Id)" class="btn btn-xs btn-warning">' + $filter('translate')('common.button.update') +'</button></div>',
                  width: 60,
                  enableSorting: false
              }
            ],
        ...
        $scope.saveDealerDiscount = function () {
            if ($scope.validateDealerDiscount()) {
                //good to go
                swal({ title: $filter('translate')('common.processing'), text: $filter('translate')('pages.dealerDiscountManagement.dealerDiscountInformation'), showConfirmButton: false });
                ...
                $http({
                    ...
                    if (data === "ok") {
                        swal($filter('translate')('common.alertTitle'), $filter('translate')('pages.dealerDiscountManagement.discountChanges'), "success");
        ...
        $scope.validateDealerDiscount = function () {
            ...
            if (isvalid) {
                if ($scope.dealerDiscountPart.startDate > $scope.dealerDiscountPart.endDate) {
                    ...
                    $scope.validate_part_startDate = "has-error";
                    isvalid = false;
                    customErrorMessage($filter('translate')('pages.dealerDiscountManagement.startGrater'));
                } else {
                    $scope.validate_part_endDate = "";
                    $scope.validate_part_startDate = "";
                }
        ...
        $scope.loadDealerDiscountForUpdate = function (dealerDiscountId) {
            if (isGuid(dealerDiscountId)) {
                swal({ title: $filter('translate')('common.loading'), text: $filter('translate')('pages.dealerDiscountManagement.dealerDiscountInformation'), showConfirmButton: false });
                $http({
                    method: 'POST',
