app.controller('DealerChequesCtrl',
    function ($scope, $rootScope, $http, ngDialog, SweetAlert, $localStorage, toaster, $filter, $translate) { //, $cookieStore, $filter
        ...
        function createGrid() {
            ...
            $scope.ColumnList = [{ field: "ClaimBatchGroupId", displayName: "", width: 0, visible: false },
                {
                    field: "BatchNumber",
                    displayName: $filter('translate')('pages.dealerCheques.batchNo'),
                    width: 250
                },
                {
                    field: "BatchDate",
                    displayName: $filter('translate')('pages.dealerCheques.batchDate'),
                    width: 200,
                    type: 'date',
                ...
                {
                    field: "GroupName",
                    displayName: $filter('translate')('pages.dealerCheques.group'),
                    width: 300
                },
                {
                    field: "Amount",
                    displayName: $filter('translate')('pages.dealerCheques.amount'),
                    width: 250,
                    cellFilter: 'number: 2'
                }];
            $scope.gridOptions = {
        ...
        function validate() {
            ...
            if (!($scope.ClaimChequePayment.CountryId != "00000000-0000-0000-0000-000000000000" && $scope.ClaimChequePayment.CountryId != "" && $scope.ClaimChequePayment.DealerId != "00000000-0000-0000-0000-000000000000" && $scope.ClaimChequePayment.DealerId != "")) {
                retVal = false;
                customErrorMessage($filter('translate')('pages.dealerCheques.errorMessages.selectCountry'));
            ...
            if (retVal) {
                if (!($scope.ClaimChequePayment.ChequeNo != "" && $scope.ClaimChequePayment.ChequeAmount != "" && $scope.ClaimChequePayment.ChequeAmount != 0 && $scope.ClaimChequePayment.ChequeDate != "" && $scope.ClaimChequePayment.IssuedDate != "")) {
                    retVal = false;
                    customErrorMessage($filter('translate')('pages.dealerCheques.errorMessages.enterCheque'));
            ...
            if (retVal) {
                if ($scope.IsExistsCheque) {
                    retVal = false;
                    customErrorMessage($filter('translate')('pages.dealerCheques.errorMessages.chequeNoExist'));
            ...
            if (retVal) {
                ...
                if ($scope.ClaimChequePayment.ClaimChequePaymentDetails.length == 0) {
                    retVal = false;
                    customErrorMessage($filter('translate')('pages.dealerCheques.errorMessages.pleaseSelectOneRow'));
            ...
            if (retVal) {
                if ($scope.ClaimChequePayment.ChequeAmount != $scope.ClaimChequePayment.Total) {
                    retVal = false;
                    customErrorMessage($filter('translate')('pages.dealerCheques.errorMessages.chequeAmount'));
        ...
        $scope.Save = function () {
            if (validate()) {
                ...
                $http({
                    ...
                    swal.close();
                    if (data == "OK") {
                        customInfoMessage($filter('translate')('pages.dealerCheques.sucessMessages.successfullySave'));
                        clear();
                    }
                    else { customErrorMessage(data); }
                }).error(function (data, status, headers, config) {
                    swal.close();
                    customErrorMessage($filter('translate')('pages.dealerCheques.errorMessages.saveFailed'));
                });
            }
        ...
        $scope.gridOptionsSearch = {
            ...
            columnDefs: [
              { name: 'Id', field: 'Id', enableSorting: false, visible: false, cellClass: 'columCss' },
                {
                    name: $filter('translate')('pages.dealerCheques.chequeNo'),
                    field: 'ChequeNo',
                    enableSorting: false,
                    cellClass: 'columCss'
                },
                {
                    name: $filter('translate')('pages.dealerCheques.chequeDate'),
                    field: 'ChequeDate',
                    enableSorting: false,
                    cellClass: 'columCss'
                },//, type: 'date', cellFilter: 'date:\'dd-MM-yyyy\'' },
                {
                    name: $filter('translate')('pages.dealerCheques.chequeAmount'),
                    field: 'ChequeAmount',
                    enableSorting: false,
                    cellClass: 'columCss'
                },
              {
                  name: ' ',
                  cellTemplate: '<div class="center"><button ng-click="grid.appScope.loadCheque(row.entity.Id)" class="btn btn-xs btn-warning">' + $filter('translate')('pages.dealerCheques.view') +'</button></div>',
                  width: 60,
                  enableSorting: false
              }
            ],
