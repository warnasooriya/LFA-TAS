    ...
    function ($scope, $rootScope, $http, SweetAlert, $localStorage, ngDialog, FileUploader, toaster) {
        ...
        $scope.Save = function () {
            if (SetValues()) {
                ...
                $http({
                    ...
                    data: { "BulkData": $scope.DataList, "CommodityType": $scope.CommodityType.CommodityTypeDescription, "Product": $scope.Product.Productcode },
                    headers: { 'Authorization': $localStorage.jwt == undefined ? jwt : $localStorage.jwt }
                }).success(function (data, status, headers, config) {
                    swal.close();
                    if (data == "Error !") {
        ...
        $scope.downloadAttachment = function () {
            ...
            $http({
                ...
                data: { "commodityCode": $scope.CommodityType.CommodityCode,  'productCode': $scope.Product.Productcode},
                headers: { 'Authorization': $localStorage.jwt == undefined ? jwt : $localStorage.jwt }
                ...
                if (data.length != 0) {
                    try {
                        ...
                        switch ($scope.CommodityType.CommodityCode) {
                            case "A": fileName = $scope.GetAutomobileTemplateFileName($scope.CommodityType.CommodityTypeDescription); break;
        ...
        $scope.GetAutomobileTemplateFileName = function(commodityName)
        {
            if(commodityName.toLowerCase() == "tyre")
            {
