app.controller('BordxViewManagementCtrl',
    function ($scope, $rootScope, $http, SweetAlert, $localStorage, ngDialog, uiGridConstants, ngTableParams, $location, toaster, $filter, $translate) {
        ...
        $scope.currentProductId = emptyGuid();
        ...
        $scope.loadDealers = function () {
            ...
            $http({
                method: 'POST',
                url: '/TAS.Web/api/DealerManagement/GetDealersByBordx',
                data: requestData,
                headers: { 'Authorization': $localStorage.jwt == undefined ? jwt : $localStorage.jwt }
            }).success(function (data, status, headers, config) {
        ...
        function loadBordxTemplates() {
            $http({
                method: 'POST',
                url: '/TAS.Web/api/Bordx/GetBordxReportTemplates',
                data: bordxTemplateCriteria,
                headers: { 'Authorization': $localStorage.jwt == undefined ? jwt : $localStorage.jwt }
            }).success(function (data, status, headers, config) {
        ...
        $scope.loadBordx = function (bordxId, year, month, country, commodityType, Number, ProductId) {
            $scope.isExport = false;
            if (isGuid(bordxId)) {
                ...
                bordxTemplateCriteria.ProductId = ProductId;
                ...
                SweetAlert.swal({
                    ...
                    text: $filter('translate')('pages.bordxViewManagement.errorMessages.invalidBordx'),
        ...
        $scope.bordxSearchGrid = {
            ...
            columnDefs: [
                ...
                { name: $filter('translate')('pages.bordxViewManagement.commodity'), field: 'CommodityType', enableSorting: false, cellClass: 'columCss' },
                //{ name: 'Country', field: 'Country', enableSorting: false, cellClass: 'columCss' },
                { name: $filter('translate')('pages.bordxViewManagement.bordxSearchPopup.insurer'), field: 'Insurer', enableSorting: false, cellClass: 'columCss' },
                { name: $filter('translate')('pages.policyCommonData.product'), field: 'Product', enableSorting: false, cellClass: 'columCss', width: 85 },
                { name: $filter('translate')('pages.bordxViewManagement.reinsurer'), field: 'Reinsurer', enableSorting: false, cellClass: 'columCss' },
                { name: $filter('translate')('pages.bordxViewManagement.bordxSearchPopup.year'), field: 'Year', enableSorting: false, cellClass: 'columCss', width: 75 },
                { name: $filter('translate')('pages.bordxViewManagement.bordxSearchPopup.month'), field: 'Month', enableSorting: false, cellClass: 'columCss', width: 75 },
                { name: $filter('translate')('pages.bordxViewManagement.no'), field: 'Number', enableSorting: false, cellClass: 'columCss', width: 75 },
                { name: $filter('translate')('pages.bordxViewManagement.startDate'), field: 'StartDate', enableSorting: false, cellClass: 'columCss' },
                { name: $filter('translate')('pages.bordxViewManagement.endDate'), field: 'EndDate', enableSorting: false, cellClass: 'columCss' },


                {
                    name: ' ',
                    cellTemplate: '<div class="center"><button ng-click="grid.appScope.loadBordx(' +
                        'row.entity.Id,row.entity.Year,row.entity.Month,row.entity.Country,row.entity.CommodityType,row.entity.Number,row.entity.ProductId)" ' +
            ...
            if ($rootScope.UserType == "RI") {
                ...
                $http({
                    ...
                    if (data.Id == emptyGuid()) {
                        SweetAlert.swal({
                            title: $filter('translate')('common.alertTitle'),
                            text: $filter('translate')('pages.bordxViewManagement.errorMessages.noDealer'),
                            confirmButtonText: $filter('translate')('common.button.ok'),
                            type: "warning",
                            confirmButtonColor: "rgb(221, 107, 85)"
                        });
        ...
        $scope.directExportAsExcel = function () {
            if ($scope.validate() && isGuid($scope.currentBordxId) && isGuid($scope.bordxReportTemplateId)) {
                swal({ title: $filter('translate')('pages.bordxViewManagement.inforMessages.pleaseWait'), text: $filter('translate')('pages.bordxViewManagement.inforMessages.preparing'), showConfirmButton: false });
                $scope.BordexExportRequest =
                    {
                        bordxId: $scope.currentBordxId,
                        userId: $rootScope.LoggedInUserId,
                        bordxReportTemplateId: $scope.bordxReportTemplateId,
                        dealerId: $scope.bordxDealerId
                    }
                $http({
                    method: 'POST',
        ...
        $scope.validate = function () {
            ...
            if ($scope.bordxReportTemplateId == undefined || $scope.bordxReportTemplateId == null || $scope.bordxReportTemplateId == "" || (!isGuid($scope.bordxReportTemplateId))) {
                isValid = false;
                customErrorMessage($filter('translate')('pages.bordxViewManagement.inforMessages.reportTemplate'))
            }
            return isValid;
        }
