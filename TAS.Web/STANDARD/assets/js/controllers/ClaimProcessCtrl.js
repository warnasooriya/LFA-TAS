...
app.controller('ClaimProcessCtrl',
    function ($scope, $rootScope, $http, ngDialog, $location, SweetAlert, $localStorage, $cookieStore, $filter, toaster, FileUploader, $state, ngTableParams, $stateParams) {
        ...
        $scope.selectedFaultChanged = function () {
            if (isGuid($scope.currentPart.fault)) {
                $http({
                    ...
                    $scope.currentPart.causeOfFailureId = "";
                    //$scope.currentPart.causeOfFailureId = $scope.faultCauseOfFailure.FaultId;
                    if (data.length === 0) {
        ...
        $scope.goodWillChanging = function () {
                    ...
                    if ($scope.currentPart.goodWillType === "P") {
                        if (parseFloat($scope.currentPart.goodWillValue) && parseFloat($scope.currentPart.goodWillValue) > 0) {
                            ...
                            if (parseFloat($scope.currentPart.goodWillValue) > 100) {
                                ...
                                $scope.currentPart.goodWillAmount = totalPriceOfPart - (totalPriceOfPart * parseFloat($scope.currentPart.goodWillValue) / 100);
                            }
                        } else {
                        ...
                        if (parseFloat($scope.currentPart.goodWillValue) && parseFloat($scope.currentPart.goodWillValue) > 0) {
                            $scope.currentPart.goodWillAmount = (parseFloat($scope.currentPart.unitPrice) * parseInt($scope.currentPart.partQty)) - parseFloat($scope.currentPart.goodWillValue);
                        } else {
                            $scope.currentPart.goodWillAmount = 0.00;
                            $scope.currentPart.goodWillValue = 0.00;
                        }
            ...
            if (parseFloat($scope.currentPart.goodWillAmount) != 0 && parseFloat($scope.currentPart.discountAmount) != 0) {
                $scope.currentPart.nettAmount = parseFloat($scope.currentPart.goodWillAmount) - parseFloat($scope.currentPart.discountAmount);
            } else if (parseFloat($scope.currentPart.goodWillAmount) == 0 && parseFloat($scope.currentPart.discountAmount) != 0) {
                $scope.currentPart.nettAmount = (parseFloat($scope.currentPart.unitPrice) * parseInt($scope.currentPart.partQty)) - parseFloat($scope.currentPart.discountAmount);
            } else if (parseFloat($scope.currentPart.goodWillAmount) != 0 && parseFloat($scope.currentPart.discountAmount) == 0) {
                $scope.currentPart.nettAmount = parseFloat($scope.currentPart.goodWillAmount) - parseFloat($scope.currentPart.discountAmount);
            } else {
               // $scope.currentPart.nettAmount = parseFloat($scope.currentPart.goodWillAmount) - parseFloat($scope.currentPart.discountAmount);
        ...
        $scope.discountChanging = function () {
                    ...
                    if ($scope.currentPart.discountType == "P") {
                        if (parseFloat($scope.currentPart.discountValue) && parseFloat($scope.currentPart.discountValue) > 0) {
                            ...
                            /*  $scope.currentPart.discountAmount = totalPriceOfPart * parseFloat($scope.currentPart.discountValue) / 100;*/
                                $scope.currentPart.discountAmount = (totalPriceOfPart -(totalPriceOfPart * parseFloat($scope.currentPart.goodWillValue) / 100)) * parseFloat($scope.currentPart.discountValue) / 100;
                            }
                        } else {
            ...
            if (parseFloat($scope.currentPart.goodWillAmount) != 0 && parseFloat($scope.currentPart.discountAmount) != 0) {
                ...
                //$scope.currentPart.nettAmount = (parseFloat($scope.currentPart.unitPrice) * parseInt($scope.currentPart.partQty)) - parseFloat($scope.currentPart.discountAmount);
                $scope.currentPart.nettAmount = parseFloat($scope.currentPart.goodWillAmount);
            } else {
                $scope.currentPart.nettAmount = parseFloat($scope.currentPart.unitPrice) * parseInt($scope.currentPart.partQty);
        ...
        $scope.addPart = function () {
            if ($scope.validatePartDetails()) {
                if (parseInt($scope.labourCharge.hours) > 0) {
                    if ($scope.validateLabourPayments()) {

                    } else {
                        customErrorMessage("Please correct highlighted fields in labour charges tab.");
                        $scope.isLabourchargeTabActive = true;
                        return;
                    }
                }
                ...
                if ($scope.IsEndorsement) {
                    ...
                    $http({
                        ...
                        if (data.Status === "ok") {
                            ...
                            if (data.ServerId != null && data.ServerId != emptyGuid()) {
                                $scope.currentPart.serverId = data.ServerId;
                            }
                            //remove if updated
                            var index = 0;
                            if ($scope.updatePartMode) {
            ...
            angular.forEach(claimItemList, function (claimItem) {
                ...
                if (claimItem.itemType === "P") {
                    ...
                    for (var i = 0; i < originalClaimItemList.length; i++) {
                        ...
                        if (originalClaimItemList[i].parentId === claimItem.partId && originalClaimItemList[i].itemType === "L") {
        ...
        $scope.syncWithExtendedGridData = function (labourCharge, part) {
            ...
            $scope.calculateTotalRequestedAndAuthorizedAmount();
                    ...
                    if (claimItem.id === itemId) {
                        ...
                        if (claimItem.partName !== "Sundry") {
                            ...
                            $scope.currentPart = {
                                ...
                                causeOfFailureId: claimItem.causeOfFailureId == null ? "" : claimItem.causeOfFailureId,
        ...
        $scope.itemStatusChanged = function () {
                if ($scope.currentPart.itemStatus == 'R') {
                    $scope.currentPart.authorizedAmount = 0.00;
                }
        }
        ...
        $scope.selectClaimToProcess = function (claimId, policyId) {
            ...
            if (isGuid(claimId)) {
                ...
                $http({
                    ...
                    if (data.ProductCode.toUpperCase() == "ILOE") {
                        $scope.isProductIloe = true;
                    } else if (data.ProductCode.toUpperCase() == "TYRE") {
                        $scope.isProductTire = true;
                    }
                  ...
                  //  $scope.claimProcess.authorizedClaimAmount = data.AuthorizedClaimAmount;
                    ...
                    $scope.claimProcess.FailureMilege = data.FailureMilege;
                    $scope.claimProcess.FailureDate = data.FailureDate;
                    var tempAuthAmount=0.00;
                    angular.forEach($scope.claimProcess.claimItemList, function (value) {
                        tempAuthAmount = tempAuthAmount + value.authorizedAmount;

                    });
                    $scope.claimProcess.authorizedClaimAmount = tempAuthAmount;
                    ...
                    if (data.ClaimStatus == 'REV') {
                       // $scope.claimProcess.claimStatus = data.ClaimStatus;
                        $scope.claimProcess.claimStatus = 'NA';
                    }
                    else if (data.ClaimStatus == 'PEN') {
                        $scope.claimProcess.claimStatus = data.ClaimStatus;
                        $scope.claimProcess.comment = data.lastComment;
                    }
                    else {
                        $scope.claimProcess.claimStatus = 'NA';
                    }
                    ...
                    $http({
                        method: 'POST',
                        url: '/TAS.Web/api/Claim/GetClaimAndPolicyAttachmentsByClaimId',
                        data: { "Id": claimId },
                        headers: { 'Authorization': $localStorage.jwt === undefined ? jwt : $localStorage.jwt }
                    }).success(function (data, status, headers, config) {
                        ...
                        for (var i = 0; i < data.Attachments.length; i++) {
                            ...
                            var attachment = {
                                ...
                                claimNumber: data.Attachments[i].ClaimNumber
                            }
        ...
        $scope.calculateTotalRequestedAndAuthorizedAmount= function(){
            var tAmount = 0.00;
            var tempRequestAmount = 0.00;
            angular.forEach($scope.extendedClaimItems, function (value) {
                tAmount = tAmount + parseFloat(value.authorizedAmount);
                tempRequestAmount = tempRequestAmount + parseFloat(value.nettAmount) + parseFloat(value.l_nettAmount);

            });
            $scope.claimProcess.authorizedClaimAmount = tAmount;
            $scope.claimProcess.totalClaimAmount = tempRequestAmount;
            $scope.totalauthorizedAmount = tempRequestAmount;
        }
            ...
            if (isValid) {
                ...
                if (isValid) {
                    if ($scope.Products[0].Productcode == "TYRE") {
                        ...
                        if ($scope.claimProcess.claimStatus=="PEN") {
                            isProcessed = true;
                            isValid = true;
                        } else {
                        angular.forEach($scope.extendedClaimItems, function (claimItem) {
        ...
        $scope.goodWillChangingLabour = function () {
                ...
                if ($scope.labourCharge.goodWillType === "P") {
                    if (parseFloat($scope.labourCharge.goodWillValue) && parseFloat($scope.labourCharge.goodWillValue) > 0) {
                        if (parseFloat($scope.labourCharge.goodWillValue) > 100) {
                            ...
                            //$scope.labourCharge.goodWillAmount = totalAmount - (totalAmount * parseFloat($scope.labourCharge.goodWillValue) / 100);
            ...
            if ($scope.labourCharge.chargeType === "H") {
                //$scope.labourCharge.nettAmount = $scope.labourCharge.goodWillAmount - parseFloat($scope.labourCharge.discountAmount);
                ...
                //-----------------------------------------------------------------------------------------------------
                if (parseFloat($scope.labourCharge.goodWillAmount) != 0 && parseFloat($scope.labourCharge.discountAmount) != 0) {
                    $scope.labourCharge.nettAmount = parseFloat($scope.labourCharge.goodWillAmount) - parseFloat($scope.labourCharge.discountAmount);
                } else if (parseFloat($scope.labourCharge.goodWillAmount) == 0 && parseFloat($scope.labourCharge.discountAmount) != 0) {
                    $scope.labourCharge.nettAmount = (parseFloat($scope.labourCharge.hourlyRate) * parseFloat($scope.labourCharge.hours)) -
																parseFloat($scope.labourCharge.discountAmount);
                } else if (parseFloat($scope.labourCharge.goodWillAmount) != 0 && parseFloat($scope.labourCharge.discountAmount) == 0) {
                     $scope.labourCharge.nettAmount = (parseFloat($scope.labourCharge.hourlyRate) * parseFloat($scope.labourCharge.hours)) -(parseFloat($scope.labourCharge.goodWillAmount) - parseFloat($scope.labourCharge.discountAmount));
                } else {
                    // $scope.currentPart.nettAmount = parseFloat($scope.currentPart.goodWillAmount) - parseFloat($scope.currentPart.discountAmount);
                    $scope.labourCharge.nettAmount = parseFloat($scope.labourCharge.hourlyRate) * parseFloat($scope.labourCharge.hours);
                }
        ...
        $scope.discountChangingLabour = function () {
                ...
                if ($scope.labourCharge.discountType === "P") {
                    if (parseFloat($scope.labourCharge.discountValue) && parseFloat($scope.labourCharge.discountValue) > 0) {
                        if (parseFloat($scope.labourCharge.discountValue) > 100) {
                            ...
                            $scope.labourCharge.discountAmount = (totalAmount - (totalAmount * parseFloat($scope.labourCharge.goodWillValue) / 100)) * parseFloat($scope.labourCharge.discountValue) / 100;
                        }
                    } else {
            ...
            if ($scope.labourCharge.chargeType === "H") {
                ...
                if (parseFloat($scope.labourCharge.goodWillAmount) > 0) {
                     /* $scope.labourCharge.nettAmount = parseFloat($scope.labourCharge.goodWillAmount) - parseFloat($scope.labourCharge.discountAmount);*/
                    $scope.labourCharge.nettAmount = (parseFloat($scope.labourCharge.hourlyRate) * parseFloat($scope.labourCharge.hours)) - (parseFloat($scope.labourCharge.goodWillAmount) + parseFloat($scope.labourCharge.discountAmount));
                } else {
                    $scope.labourCharge.nettAmount = (parseFloat($scope.labourCharge.hourlyRate) * parseFloat($scope.labourCharge.hours)) -  parseFloat($scope.labourCharge.discountAmount);
                }
                //$scope.labourCharge.nettAmount = (parseFloat($scope.labourCharge.hourlyRate) * parseInt($scope.labourCharge.hours))
                //    - parseFloat($scope.labourCharge.goodWillAmount) - parseFloat($scope.labourCharge.discountAmount);
                $scope.currentPart.authorizedAmount = $scope.labourCharge.nettAmount + $scope.currentPart.nettAmount;
            ...
            swal({
                ...
                function () {
                    if ($location.path() === '/app/claim/process/') {
                        ...
                        $location.path('app/claim/process/');
                    }
                });
        }
        ...
        $scope.processClaim = function () {
            $scope.allItemRejected = true;
            angular.forEach($scope.extendedClaimItems, function (claimItem) {
                if (claimItem.itemStatus == "A") {
                    $scope.allItemRejected = false;
                }
            });
            if ($scope.claimProcess.claimStatus == "REJ" || $scope.claimProcess.claimStatus == "PEN" ) {
                $scope.allItemRejected = false;
            }


            if (!$scope.allItemRejected) {
                if ($scope.IsEndorsement) {
                    swal({
                        title: "Are you sure?",
                    ...
                    swal({
                        ...
                        function () {
                            ...
                            if ($scope.claimProcess.claimStatus === 'APP' || $scope.claimProcess.claimStatus === 'PEN') {
                ...
                customErrorMessage("All items are rejected. So the Claim cannot be approved.");
            }
        }
            ...
            if ($scope.validateClaimStatus() && $scope.validateClaimItems()) {
                if ($scope.validateEngineerComments()) {
                    ...
                    var data = {
                        ...
                        athorizedAmount:$scope.claimProcess.authorizedClaimAmount
                    };
        ...
        $scope.updateServiceRecord = function (value) {

            angular.forEach($scope.itemServiceRecords, function (Records) {
                if (Records.Id === value) {
                    $scope.serviceRecord.serviceNumber = Records.ServiceNumber;
                    $scope.serviceRecord.serviceDate = Records.ServiceDate;
                    $scope.serviceRecord.milage = Records.Milage;
                    $scope.serviceRecord.remarks = Records.Remarks;
                    $scope.serviceRecord.id = Records.Id;
                }
            });


        }
        $scope.deleteServiceRecord = function (value) {
            ...
            if (isGuid($scope.policyId)) {
                    ...
                    $http({
                        method: 'POST',
                        url: '/TAS.Web/api/claim/deleteServiceRecord',
                        data: {
                            "policyId": $scope.policyId,
                            "serviceData": $scope.serviceRecord
                        },
                        ...
                        if (data == 'ok') {
                            customInfoMessage("Service record deleted successfully.");
                            $scope.clearServiceRecord();
                            $scope.GetServiceHistory($scope.policyId);
                        } else {
                            if (data == '') {
                                customErrorMessage("Error occured while deleting service history");
                            } else {
                                customErrorMessage(data);
                            }
                        }
        ...
        $scope.addSundry = function () {
            if ($scope.validateSundry()) {
                var item = {
                    ...
                    "discountAmount": 0,
                    "fault": emptyGuid(),
                    "nettAmount": $scope.sundry.value,
                    "authorizedAmount": $scope.sundry.Status=='A' ?  $scope.sundry.value : 0,
                    "itemStatus": $scope.sundry.Status,
                    "serverId": $scope.sundry.serverId
                };
                var labourCharge = {
                    chargeType: 'H',
                    hourlyRate: 0.00,
                ...
                if ($scope.IsEndorsement) {
                    ...
                    $scope.syncWithExtendedGridData(labourCharge, item);
                    $scope.resetSundryDetails();
                } else {
                    var data = {
                        "claimId": $scope.claimId === '' ? emptyGuid() : $scope.claimId,
                        ...
                        "loggedInUserId": $localStorage.LoggedInUserId,
                        "dealerId": typeof $scope.dealerId === 'undefined' ? emptyGuid() : $scope.dealerId,
                        "isUpdate": $scope.updateSundry
                    };

                    swal({ title: 'Processing...', text: 'Saving Sundry Information', showConfirmButton: false });
                    $http({
                        method: 'POST',
                        url: '/TAS.Web/api/claim/SavePartWithClaim',
                        data: data,
                        ...
                        if (data.Status === "ok") {
                            ...
                            $scope.claimId = data.ClaimId;
                            if (data.ServerId != null && data.ServerId != emptyGuid()) {
                                $scope.sundry.serverId = data.ServerId;
                            ...
                            if ($scope.updateSundry) {
                                angular.forEach($scope.extendedClaimItems, function (displayItem) {
                                    ...
                                    if (displayItem.serverId === $scope.sundry.serverId) {
                                        $scope.extendedClaimItems.splice(index, 1);
                                    }
                                    index++;
                                });
                            ...
                            $scope.syncWithExtendedGridData(labourCharge, item);
                            customInfoMessage("Sundry successfully saved.");
                            $scope.resetSundryDetails();
        ...
        $scope.validateSundry = function () {
            ...
            if ($scope.sundry.Status =='') {
                $scope.validate_sundry_status = "has-error";
                isValid = false;
            } else {
                $scope.validate_sundry_status = "";
            }
        ...
        $scope.labourDiscountSchemeChanged = function () {
            var data = {
                ...
                "policyId": typeof $scope.policyId === 'undefined' ? emptyGuid() : $scope.policyId,
                "type": "L",
                "schemeId": $scope.labourCharge.labourDiscountScheme,
                "makeId": $scope.makeId,
        ...
        $scope.discountSchemaLoad = function () {
            $scope.labourCharge.labourDiscountScheme = $scope.currentPart.partDiscountScheme;
            $scope.partDiscountSchemeChanged();
            $scope.labourDiscountSchemeChanged();
        }
                    ...
                    if (claimItem.id === itemId) {
                        ...
                        if (claimItem.partName !== "Sundry") {
                            ...
                            $scope.currentPart = {
                                ...
                                serverId: claimItem.serverId,
                                partDiscountScheme: 'NA',
                                rejectionTypeId: claimItem.rejectionTypeId == undefined ? emptyGuid() : claimItem.rejectionTypeId,
                                nettAmount: parseFloat(claimItem.nettAmount.replace(/\,/g, '')),
                                authorizedAmount: parseFloat(claimItem.unitPrice.replace(/\,/g, '')),
                                itemStatus: claimItem.itemStatus == null ? "" : claimItem.itemStatus,
                                causeOfFailureId: claimItem.causeOfFailureId == null ? "" : claimItem.causeOfFailureId,
