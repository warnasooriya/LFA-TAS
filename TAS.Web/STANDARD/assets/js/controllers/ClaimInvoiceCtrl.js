    ...
    function ($scope, $rootScope, $http, ngDialog, $localStorage, SweetAlert, toaster, $filter, FileUploader) {
        ...
        $scope.gridOptions = {
            ...
            columnDefs: [
                ...
                {
                    name: '   ',
                    cellTemplate: '<div class="center"><button ng-click="grid.appScope.loadInvoiceDataById(row.entity)" class="btn btn-xs btn-info">   adjust   <a ng-show="row.entity.IsBatching==true" class="fa fa-check" title="Adjusted Claim"> </a></button></div>',
                    width: 60,
                    enableSorting: false
                }
                //{
        ...
        $scope.buttonVisibilityForSaveUpdateConfirm = function () {
            ...
            //$scope.ClaimInvoiceEntry.InvoiceAmount = "";
            //$scope.ClaimInvoiceEntry.Id = "00000000-0000-0000-0000-000000000000";
            var isBatchClaims = $scope.mySelectedRows.filter(a => a.IsBatching == true);
            ...
            // check all claims adjust with selected invoice
            var isAdjustAllClaimsContainsThisInvoiceNo = false;
            if (len > 0) {
                var invNo = $scope.mySelectedRows[0].InvoiceNumber;
                var allClaimsWithInvoiceNo = $scope.claims.filter(a => a.InvoiceNumber == invNo);
                var AdjustClaimsWithInvNo = $scope.claims.filter(a => a.InvoiceNumber == invNo && a.IsBatching == true);
                if (allClaimsWithInvoiceNo.length == AdjustClaimsWithInvNo.length) {
                    isAdjustAllClaimsContainsThisInvoiceNo = true;
                } else {
                    customWarningMessage("Please Adjust All  Claims Contain With " + invNo + " invoice number  before Confirm .");
                    return false;
                }

            }

            if (len > 0 && sameInvoiceNumber.length == len && isBatchClaims.length == len && isAdjustAllClaimsContainsThisInvoiceNo==true) {
                $scope.readyToConfirm = true;
                $scope.claimInvoiceSisableConfirm = false;
            } else {

                if (isBatchClaims.length < $scope.mySelectedRows.length ) {
                    $scope.readyToConfirm = false;
                    $scope.claimInvoiceSisableConfirm = true;
                    customWarningMessage("Please Adjust Claim before Confirm .");
                    return false;
                }
        ...
        $scope.submitAdjustAmount = function () {
            if ($scope.adjustment.claimIdAj != emptyGuid()) {
                ...
                if ($scope.adjustPartAmount == undefined || $scope.adjustPartAmount == null || $scope.adjustPartAmount == "") {
                    $scope.adjustPartAmount = 0;
                }
                if ($scope.adjustLabourAmount == undefined || $scope.adjustLabourAmount == null || $scope.adjustLabourAmount == "") {
                    $scope.adjustLabourAmount = 0;
                }
                if ($scope.adjustSundryAmount == undefined || $scope.adjustSundryAmount == null || $scope.adjustSundryAmount == "") {
                    $scope.adjustSundryAmount = 0;
                }
                data = {
