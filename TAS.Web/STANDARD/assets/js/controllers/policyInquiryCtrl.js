app.controller('PolicyInquiryCtrl', function ($scope, $rootScope, $http, ngDialog, SweetAlert,
    ...
    $scope.DealerAttachements = [];
    $scope.AdminAttachements = [];
    ...
    $scope.LogedInAdminUser = false;
    $scope.isTyrebyAutomobile = false;
    $scope.isTyrePolicyDetails = false;
    ...
    if ($localStorage.UserType != undefined && $localStorage.UserType != null && $localStorage.UserType == "IU") {
        $scope.LogedInAdminUser = true;
    }
    ...
    $scope.loadTyreDetailsToVisualizer = function (policyBundle,tyreDetails, allTyreSame) {
        ...
        if (policyBundle.length > 0) {
            $scope.tireSizesAvailable = true;
            let tireDetails = tyreDetails;
    ...
    $scope.loadPolicy = function (policyId) {
        ...
        if (isGuid(policyId)) {
            ...
            $http({
                ...
                if (data.policyDetails.MWIsAvailable) {
                    $scope.Policy.MWStartDate = data.policyDetails.MWStartDate;
                    $scope.Policy.MWEndDate = data.policyDetails.MWEndDate;
                } else {
                    $scope.Policy.MWStartDate = "N/A";
                    $scope.Policy.MWEndDate = "N/A";
                }


                $scope.Policy.GrossWeight = data.policyDetails.GrossWeight;
                $scope.Policy.EngineNumber = data.policyDetails.EngineNumber;
                if ($scope.Policy.Product == 'ILOE' || $scope.Policy.Product=='Involuntary Loss of Employment') {

                    $scope.isProductILOE = true;
                }

                if (data.policyDetails.ProductCode == "ADT") {
                    $scope.isTyrePolicyDetails = true;
                    $scope.additionalPolicyData = data.policyDetails.AdditionalTyreDetails[0];
                }

                $scope.Policy.Status = data.policyDetails.Status;
                $scope.retBundle = data.policyDetails.retBundle;
                $scope.TireDetails = data.policyDetails.TireDetails;

                // Tyre data loading part

                if (data.policyDetails.CommodityType == "Tyre" && data.policyDetails.CommodityTypeCode == "O") {
                    $scope.isTyre = true;
                    $scope.Policy.AditionalModel = data.policyDetails.AditionalModel;
                    $scope.Policy.AditionalMake = data.policyDetails.AditionalMake;
                    $scope.Policy.AditionalMilage = data.policyDetails.AditionalMilage;
                    $scope.Policy.AditionalModelYear = data.policyDetails.AditionalModelYear;
                    $scope.Policy.AditionalCity = data.policyDetails.AditionalCity;

                    $scope.loadTyreDetailsToVisualizer($scope.retBundle, data.policyDetails.TireDetails, data.policyDetails.AllTyresAreSame);

                    angular.forEach($scope.retBundle, function (value) {
                        ...
                        if ($scope.TireDetails.length == 1) {

                            $scope.serialno = $scope.TireDetails[0].Position + "  -  " + $scope.TireDetails[0].SerialNumber;
                            $scope.Policy.BAndW.articleNumberV1 = $scope.TireDetails[0].ArticleNumber;
                        }

                        if ($scope.TireDetails.length == 2) {
                            if ($scope.TireDetails.length == 2) {

                                $scope.serialno = $scope.TireDetails[0].Position + "  -  " + $scope.TireDetails[0].SerialNumber;
                                $scope.serialno1 = $scope.TireDetails[1].Position + " - " + $scope.TireDetails[1].SerialNumber;
                                $scope.SerialNoV2disable = false;
                            } else if ($scope.TireDetails.length == 3) {
                                $scope.serialno = $scope.TireDetails[0].Position + "  -  " + $scope.TireDetails[0].SerialNumber;
                                $scope.serialno1 = $scope.TireDetails[1].Position + " - " + $scope.TireDetails[1].SerialNumber;
                                $scope.serialno2 = $scope.TireDetails[2].Position + " - " + $scope.TireDetails[2].SerialNumber;

                                $scope.Policy.BAndW.articleNumberV1 = $scope.TireDetails[0].ArticleNumber;
                                $scope.Policy.BAndW.articleNumberV2 = $scope.TireDetails[2].ArticleNumber;
                                $scope.SerialNoV2disable = true;
                            } else if ($scope.TireDetails.length == 4) {
                                $scope.serialno = $scope.TireDetails[0].Position + "  -  " + $scope.TireDetails[0].SerialNumber;
                                $scope.serialno1 = $scope.TireDetails[1].Position + " - " + $scope.TireDetails[1].SerialNumber;
                                $scope.serialno2 = $scope.TireDetails[2].Position + " - " + $scope.TireDetails[2].SerialNumber;
                                $scope.serialno3 = $scope.TireDetails[3].Position + " - " + $scope.TireDetails[3].SerialNumber;
                                $scope.Policy.BAndW.articleNumberV1 = $scope.TireDetails[0].ArticleNumber;
                                $scope.Policy.BAndW.articleNumberV2 = $scope.TireDetails[2].ArticleNumber;
                                $scope.SerialNoV2disable = true;
                            }
                            ...
                            else if ($scope.TireDetails.length != 2 && $scope.TireDetails[0].Diameter == $scope.TireDetails[2].Diameter &&
                                $scope.TireDetails[0].Width != $scope.TireDetails[2].Width && $scope.TireDetails[0].CrossSection != $scope.TireDetails[2].CrossSection
                                && $scope.TireDetails.LoadSpeed != $scope.TireDetails[2].LoadSpeed) {
                                $scope.serialno = $scope.TireDetails[0].Position + "  -  " + $scope.TireDetails[0].SerialNumber;
                                $scope.serialno1 = $scope.TireDetails[1].Position + " - " + $scope.TireDetails[1].SerialNumber;
                                $scope.serialno2 = $scope.TireDetails[2].Position + " - " + $scope.TireDetails[2].SerialNumber;
                                $scope.serialno3 = $scope.TireDetails[3].Position + " - " + $scope.TireDetails[3].SerialNumber;
                ...
                } else if (data.policyDetails.CommodityType == "Tyre" && data.policyDetails.CommodityTypeCode == "A") {
                    $scope.isTyrebyAutomobile = true;
                }
                ...
                $http({
                    method: 'POST',
                    url: '/TAS.Web/api/PolicyReg/GetAttachmentsByPolicyIdByUserType',
                    data: { "Id": policyId },
                    headers: { 'Authorization': $localStorage.jwt === undefined ? jwt : $localStorage.jwt }
                }).success(function (data, status, headers, config) {
                    $scope.DealerAttachements = [];
                    $scope.AdminAttachements = [];
                    $scope.attachments_temp = data.DealerAttachments;


                    for (var i = 0; i < data.DealerAttachments.length; i++) {
                        var file = {
                            name: data.DealerAttachments[i].FileName,
                            size: data.DealerAttachments[i].AttachmentSizeKB
                        }
                        var attachment = {
                            documentType: data.DealerAttachments[i].DocumentType,
                            id: data.DealerAttachments[i].Id,
                            file: file,
                            ref: data.DealerAttachments[i].FileServerRef,
                            attachmentSection: data.DealerAttachments[i].AttachmentSection
                        }
                        $scope.DealerAttachements.push(attachment);
                        $scope.itemDocUploader.queue.push(attachment)

                    }

                    for (var j = 0; j < data.AdminAttachments.length; j++) {
                        var file = {
                            name: data.AdminAttachments[j].FileName,
                            size: data.AdminAttachments[j].AttachmentSizeKB
                        }
                        var attachment = {
                            documentType: data.AdminAttachments[j].DocumentType,
                            id: data.AdminAttachments[j].Id,
                            file: file,
                            ref: data.AdminAttachments[j].FileServerRef,
                            attachmentSection: data.AdminAttachments[j].AttachmentSection
                        }
                        $scope.AdminAttachements.push(attachment);
                        $scope.itemDocUploader.queue.push(attachment);
