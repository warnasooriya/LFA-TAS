...
b.year                                                                AS
BordxYear,
CONVERT(VARCHAR,b.year) + REPLICATE('0',2-LEN(b.month)) + CONVERT(VARCHAR,b.month)     AS
BDXMTH,
c.dealname                                                            AS
DealName,
dt.NAME                                                               AS
...
END                                                                   AS VehicleRegistrationDate,
...
p.LocalCurrencyConversionRate                                         AS ConversionRateUS,
'N/A'                                                                 AS
StartDateRSA,
'N/A'                                                                 AS
TerminationDateRSA,
...
ceid.InvoiceNumber											     as InvoiceNumber,
ic.PlateNumber													as PlateNumber,
apmd.MakeName													as MakeName,
apmodeld.ModelName												as ModelName,
ceid.AdditionalDetailsModelYear									as AModelYear,
ceid.AdditionalDetailsMileage									as AdditionalMileage,
ic.TireQuantity													as NoofTires,
-- inctd.ArticleNumber+''+	atsp.Pattern											as ArticleNumber,
 STUFF((
          SELECT distinct ',' + i_inctd.ArticleNumber+''+	i_atsp.Pattern
        from policy i_p
		inner join Contract i_c ON i_p.ContractId=i_c.id
 		LEFT JOIN InvoiceCodeDetails i_icd
				ON i_icd.PolicyId = i_p.id
		LEFT JOIN InvoiceCode i_ic
				ON i_ic.id = i_icd.InvoiceCodeId
	    LEFT JOIN InvoiceCodeTireDetails i_inctd
				ON i_inctd.InvoiceCodeDetailId = i_icd.id
		LEFT JOIN AvailableTireSizesPattern i_atsp
				ON i_atsp.Id = i_inctd.AvailableTireSizesPatternId
				where i_p.id=p.id
          FOR XML PATH('')), 1, 1, '') as ArticleNumber ,
rec.ContractNo,
c.LiabilityLimitation as Deposit ,
vd.dealerPrice *  p.LocalCurrencyConversionRate as FinanceAmount,
il.Months as LonePeriod,
il.Months as PeriodOfCover,
p.MonthlyEMI as MonthlyEMI,
...
Upper(con.countrycode + '-'
      ...
      + RIGHT('00000000'+Cast(p.uniqueref AS VARCHAR), 7))            AS
SystemGeneratedNum,
p.PolicySoldDate                                                      As  PolicySoldDate, --policy history data not capturing
...
p.MWStartDate as ManfWarrantyStartDate,
Dateadd(day, -1,
...
END                                                                  as MileageLimitationInKMs,
Dateadd(month, Isnull(mw.warrantymonths, 0), p.MWStartDate)
	as DateOfInsuranceRiskStart,
	Dateadd(day, -1,Dateadd(month,Isnull(il.Months, 0), Dateadd(month, Isnull(mw.warrantymonths, 0), p.MWStartDate)))
...
+ Isnull((
	SELECT
		CASE
			WHEN ccm.ispercentage=1 THEN ccm.commission*
		(CASE WHEN pboGross.code='RP' THEN
		 CASE WHEN ct.commoditycode='A' THEN vd.dealerprice*
		(CASE WHEN ccm.isonnrp=1 THEN p.NRP
		 ELSE p.Premium
		 END) /100 WHEN
