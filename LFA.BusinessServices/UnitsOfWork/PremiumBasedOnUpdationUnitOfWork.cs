...
{
    internal sealed class PremiumBasedOnUpdationUnitOfWork : UnitOfWork
    {
        public PremiumBasedOnRequestDto PremiumBasedOn;
        public PremiumBasedOnResponseDto ExPr;
        private string UniqueDbName = string.Empty;
        public PremiumBasedOnUpdationUnitOfWork(PremiumBasedOnRequestDto PremiumBasedOn)
        {
                ...
                if (dbName != "")
                {
                    ...
                    if (tasTpaConnString != "")
                    {
                        ...
                        if (JWTHelper.checkTokenValidity(Convert.ToInt32(ConfigurationData.tasTokenValidTime.ToString())))
                        {
                            ...
                            if (ce.IsPremiumBasedOnExists == true)
                            {
                                PremiumBasedOn.EntryDateTime = ce.EntryDateTime;
                                PremiumBasedOn.EntryUser = ce.EntryUser;
                                return true;
                            }
                            else
                            {
        ...
        public override void Execute()
        {
            try
            {
                ...
                //if (ce.IsPremiumBasedOnExists == true)
                //{
                    bool result = PremiumBasedOnEntityManager.UpdatePremiumBasedOn(PremiumBasedOn);
                    if (result)
                    {
                        /* remove cache */
